...
```{r}
data(nottem)
plot.ts(nottem,xlab='temps',ylab='température', main = "Trajectoire de la série de données nottem")
```
...
par(mfrow = c(1, 2))
plot(fit$trend, ylab = "Température", xlab = "Temps en années", main = "Tendance de la série")
plot(fit$trend, ylab = "Température", xlab = "Temps en années",  main = "Tendance de la série", ylim = c(30,65))
```
...
```{r}
plot(fit$seasonal, main = "Saisonnalité de la série", xlab = "Temps en années")
```

Ce graphique ne laisse point de place au doute concernant la saisonnalité de la série, qui, de plus est d'ordre 12.

```{r}
plot(fit$random, ylim = c(-6,6), main = "Saisonnalité de la série", xlab = "Temps en années")
...
```{r}
plot(fit$figure, main = "Coefficients saisonniers")
mean(fit$figure)
```
...
```{r}
monthplot(nottem, xlab = "Mois de l'année", ylab = "Température", main = "Tracé de la série par mois")
```
...
points(nott_v, col = "darkgreen", lwd = 2, type = "l", ylab = "Température", xlab = "Temps en années")
```
...
plot(window(pred.sarima$mean, start = c(1937, 1)), col = "darkblue", lwd = 2,
     main = "Prévision de 1937 à 1939", xlab =  "Temps en années", ylab ="Température")
...
```{r}
plot(tnupp - tnlow, type = "o", xlab = "Temps en années")
```
